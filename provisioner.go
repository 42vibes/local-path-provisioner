...
func (p *LocalPathProvisioner) createHelperPod(action ActionType, cmd []string, o volumeOptions, cfg *StorageClassConfig) (err error) {
	...
	if err := applyQuotaToHelperPod(helperPod, cfg.Quota); err != nil {
		return err
	}
...
func commandAndScriptKeysForAction(action ActionType, setupCommand, teardownCommand string, quota QuotaConfig) ([]string, []string) {
	...
	case ActionTypeCreate:
		...
		if quota.Type == QuotaTypeXFSProject {
			return []string{"/bin/sh", filepath.Join(helperScriptDir, xfsQuotaSetupScriptKey)}, []string{xfsQuotaSetupScriptKey}
		}
	...
	case ActionTypeDelete:
		...
		if quota.Type == QuotaTypeXFSProject {
			return []string{"/bin/sh", filepath.Join(helperScriptDir, xfsQuotaTeardownScriptKey)}, []string{xfsQuotaTeardownScriptKey}
		}
...
func applyQuotaToHelperPod(helperPod *v1.Pod, quota QuotaConfig) error {
...
func canonicalizeQuotaConfig(data *QuotaConfigData, hasNodePathMap, hasSharedFileSystemPath bool) (QuotaConfig, error) {
...
func canonicalizeStorageClassConfig(data *StorageClassConfigData) (cfg *StorageClassConfig, err error) {
	...
	quota, err := canonicalizeQuotaConfig(data.Quota, len(cfg.NodePathMap) > 0, cfg.SharedFileSystemPath != "")
	if err != nil {
		return nil, err
	}
	cfg.Quota = quota
